...
def clean_acris_csv(filename):
        ...
        zipcodes = pd.read_csv('./data/zip_code_database.csv', encoding='latin-1', dtype=object, converters={'zip': str})
        curr_row_progress = 0
        logger.debug('Starting to process %s', filename)
        t = pd.read_csv(filename, chunksize=chunksize, iterator=False, dtype=object, converters={'ZIP': str}, 
        ...
        for chunk in t:
            address_extract = chunk.copy()
            address_extract['ZIP'] = address_extract['ZIP'].str[:5]

            zipcode_extract = zipcodes.loc[:,['zip', 'state','primary_city']].copy()

            address_extract = pd.merge(address_extract, zipcode_extract, how='left', left_on='ZIP', right_on='zip', suffixes=('_x', '_y'), copy=True)
            ...
            address_extract.to_csv(out_dir + os.path.basename(filename) + '.csv', index=False, header=False, mode='a', chunksize=chunksize)

            curr_row_progress += chunksize
            logger.debug('%s: Wrote out %s rows', filename, curr_row_progress)
            # Refactor this if you need addresses again - ignore for now because trying to canonicalize on just state,city,zip,name.
            ...
            #address_extract['parsed_address'] = sample.apply(lambda x: expand_address(x, languages=['en'], delete_final_periods=True, delete_acronym_periods=True)[0])
            # chunk_sub['STATE'] = address_extract['state']
            # chunk_sub['CITY'] = address_extract['primary_city']
            # print(chunk['ZIP'], chunk['new_City'])
